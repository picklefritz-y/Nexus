...
// --- Robust JSON parsing of Claude responses ---

// Scan from `start` (an opening `{` or `[`) and return the substring up to its
// matching close, respecting strings/escapes so braces inside quoted values
...
// Parse JSON from a Claude text response that may be wrapped in ```json fences
// or padded with leading/trailing prose. Falls back to the first balanced
// object/array if a straight parse fails.
...
Return ONLY valid JSON.`;
  ...
  const content = response.content[0];
  if (content.type !== "text") throw new Error("Unexpected response");
  const themes = parseJsonResponse(content.text);
...
Be conservative. Return ONLY valid JSON.`;
  ...
  const content = response.content[0];
  if (content.type !== "text") throw new Error("Unexpected response");
  const contradictions = parseJsonResponse(content.text);
...
async function storeResults(fetchResult: ExtractedContent, extraction: any, themeData: any, contradictions: any[]) {
  ...
  for (const t of themeData.themes) {
    ...
    if (t.domain) {
      ...
      // Both `name` and `slug` are unique on Domain, so a plain upsert keyed on
      // slug blows up when the name already exists under a different slug.
      // Match either field, create only if neither exists, and re-fetch if a
      // concurrent insert (or slug/name collision) trips a unique constraint.
      let domain = await prisma.domain.findFirst({ where: { OR: [{ name: t.domain }, { slug }] } });
      if (!domain) {
        try {
          domain = await prisma.domain.create({ data: { name: t.domain, slug, icon: "📂", isAutoGenerated: true } });
        } catch (_e) {
          domain = await prisma.domain.findFirst({ where: { OR: [{ name: t.domain }, { slug }] } });
        }
      }
      domainId = domain?.id;
    }
