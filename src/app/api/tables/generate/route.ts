    ...
    async start(controller) {
      try {
        ...
        const claimContext = relevantClaims.map((c: any, i: number) => {
          const sources = c.sources.map((s: any) => `${s.source.title} (${s.source.author || "Unknown"}, ${s.source.publicationDate ? new Date(s.source.publicationDate).toISOString().split("T")[0] : "n.d."})`).join("; ");
...
${contradictions.length > 0 ? `Contradicted by: ${contradictions.join("; ")}` : ""}`;
        ...
        const sourceContext = relevantSources.map((s: any, i: number) => {
          ...
          return `[SOURCE ${i + 1}] "${s.title}" by ${s.author || "Unknown"} (${s.type}, ${s.publicationDate ? new Date(s.publicationDate).toISOString().split("T")[0] : "n.d."})
${s.url ? `URL: ${s.url}` : ""}
${textExcerpt}`;
        }).join("\n\n---\n\n");
